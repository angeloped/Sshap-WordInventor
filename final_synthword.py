...
with open("./sshapwords/Sshapen.words".format(word_pos), "a") as fiel:
	for origin in os.listdir("./transliterd/"):
		...
		try:
			if len(pos_out[0].keys()) == 1:
				if pos_out[0][origin] in ("CC","CD","DT","EX","FW","IN","JJ","LS","MD","PDT","PRP","PRP$","RB","RBR","RBS","RP","TO","UH","WDT","WP","WP$","WRB","VB","NN"):
					...
					new_data = {'origin':origin, 'forged':[final_word,sshaped0], 'suggest':[suggest,sshaped1], 'relatives':sorted_words, 'tag':pos_out[0][origin]}
					#print("{0}\n".format(new_data))
					fiel.write("{0}\n".format(new_data))
					#print("origin: ", origin, " => " , final_word, sshapify(final_word.upper()), sorted_words)
					
		except Exception as errr:
			alt_out = {'origin':origin, 'forged':[sorted_words[0],sshapify(sorted_words[0].upper())], 'suggest':['',''], 'relatives':sorted_words, 'tag':pos_out[0][origin]} # alternative output
			errr_msg = "{0}: {1}\n".format(errr, alt_out)
			#print(errr_msg)
			with open("./sshapwords/error_final_synthword.log","a") as errlog:
